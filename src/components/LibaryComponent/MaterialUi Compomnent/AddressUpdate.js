...
const schema = yup.object().shape({
    ...
    phone_number: yup.string().min(10, 'Enter A Valid Phone Number').required("Phone number is required"),
    country: yup.string().required("Country is required"),
    city: yup.string().required("City is required"),
    area: yup.string().required("Area is required"),
    address: yup.string().required("Address is required"),
    zip_code: yup.string().matches(/^\d+$/, "Zip code must be numeric").required("Zip code is required"),
});
...
console.log(userDetails,"!llllllll")
        ...
        <div className="fixed inset-0 z-[9999] bg-black bg-opacity-50 flex items-center justify-center">
            <div className="max-w-lg w-[90%] bg-white rounded-lg shadow-2xl overflow-hidden">
                ...
                <div>
                    ...
                    <select p {...register("city")}  className="w-full border-2 p-2 rounded">
                    <option defaultValue="">{userDetails?.city}</option>
                        {cities?.map((city) => (
                            <option key={city?.geonameId} value={city?.geonameId}> {/* 🔹 Name store ho raha hai */}
                                {city.name}
                            </option>
                        ))}
                    </select>
                ...
                <div>
                    ...
                    <select {...register("area")} className="w-full border-2 p-2 rounded">
                        ...
                        {areas?.map((area) => (
                            <option key={area?.geonameId} value={area?.name}> {/* 🔹 Name store ho raha hai */}
                                {area.name}
                            </option>
                        ))}
                    </select>
                ...
                <div>
                    ...
                    <input {...register("address")} type="text" defaultValue={userDetails?.address} className="w-full border-2 p-2 rounded" />
                ...
                <div>
                    ...
                    <input {...register("zip_code")} type="text" defaultValue={userDetails?.zip_code} className="w-full border-2 p-2 rounded" />
                ...
                <div>
                    ...
                    <input {...register("phone_number")} type="text" defaultValue={userDetails?.phone_number} className="w-full border-2 text-black p-2 rounded" />
