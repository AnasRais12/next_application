          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex sm:flex-col sm:justify-start justify-between flex-row">
                ...
                <p className="font-medium  sm:text-start text-end">
                  {order.order_id}
                </p>
              </div>
              <div className="flex sm:flex-col sm:justify-start justify-between flex-row">
            ...
            <div className="mt-6 pl-[-6px]">
              <div
                className={` rounded-lg border-2   ${order?.items?.length == 1 ? 'h-fit' : 'h-36'}  mb-4 overflow-y-auto overflow-x-auto`}
              >
                <table className="w-full ">
                  <thead className="bg-gray-50 border-b-2">
                    <tr>
                      <th className="px-4 py-2 text-left text-sm border-r-2 font-medium text-gray-500">
                        Item
                      </th>
                      <th className="px-4 py-2 text-left text-sm font-medium border-r-2  text-gray-500">
                        Quantity
                      </th>
                  ...
                  <tbody className="divide-y  text-center w-full  ">
                    {order.items.map((item, index) => (
                      <tr className="" key={index}>
                        <td className="px-2 border-r-2 py-1 text-left w-full text-[12px]">
                          {item.product_name}
                        </td>
